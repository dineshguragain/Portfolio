    ...
    function renderPortfolio(data) {
      const { personal, skills, experience, projects, education, certifications, testimonials } = data;

      // Hero
      document.getElementById('hero-title').innerHTML =
        `Hi, I'm <span class="accent">${personal.name}</span>.<br/>${personal.title}.`;
      document.getElementById('hero-description').textContent = personal.tagline;
      ...
      if (personal.social) {
        socialLinks.innerHTML = Object.entries(personal.social).map(([key, url]) => `
          <a href="${url}" class="social-link" target="_blank" rel="noopener noreferrer" aria-label="${key}" title="${key.charAt(0).toUpperCase() + key.slice(1)}">
            ${icons[key] || key}
          </a>
        `).join('');
      }
      ...
      // Certifications
      const certsContainer = document.getElementById('about-certs');
          ...
          certsContainer.innerHTML += `
            <div class="cert-item">
              ...
              <div>
                ...
                <div class="cert-info-meta">${cert.issuer} · ${cert.year}</div>
              </div>
            </div>
          `;
        });
      ...
      skills.forEach(skill => {
        ...
        card.innerHTML = `
          <div class="skill-card-header">
            <span class="skill-card-icon">${skillIcons[skill.category] || '💡'}</span>
            ${skill.category}
          </div>
      ...
      projects.forEach(proj => {
        ...
        card.innerHTML = `
          ${proj.featured ? '<div class="project-featured-badge">★ Featured</div>' : ''}
...
contactForm.addEventListener("submit", function (e) {
    ...
    .catch(function (error) {
      ...
      if (certifications) {
        statNumbers[2].textContent = certifications.length;
      }
